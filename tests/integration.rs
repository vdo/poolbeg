...
async fn test_http_eth_chain_id_arbitrum() {
    ...
    assert_eq!(
        body["result"], "0xa4b1",
        "Arbitrum chain ID should be 42161 (0xa4b1)"
    );
}
...
async fn test_ws_subscribe_new_heads() {
    ...
    let sub_req =
        serde_json::to_string(&rpc_request("eth_subscribe", json!(["newHeads"]), 1)).unwrap();
    ...
    // Unsubscribe
    let unsub_req =
        serde_json::to_string(&rpc_request("eth_unsubscribe", json!([sub_id]), 2)).unwrap();
