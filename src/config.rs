...
fn deserialize_ttl<'de, D: serde::Deserializer<'de>>(deserializer: D) -> Result<TtlValue, D::Error> {
...
fn parse_duration_string(s: &str) -> Result<Duration> {
    ...
    let secs: u64 = s.parse().context("invalid duration format, use e.g. '3s', '250ms', '1h'")?;
...
fn deserialize_duration<'de, D: serde::Deserializer<'de>>(deserializer: D) -> Result<Duration, D::Error> {
    let s = String::deserialize(deserializer)?;
...
impl Config {
    ...
    fn validate(&self) -> Result<()> {
        anyhow::ensure!(!self.chains.is_empty(), "at least one chain must be configured");
...
"#;
        ...
        assert_eq!(config.chains[0].expected_block_time, Duration::from_secs(12));
    }
...
"#;
        ...
        let result = Config::load(tmpfile.path());
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("route must start with '/'"));
    }
}
