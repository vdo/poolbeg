...
impl CacheLayer {
    pub async fn new(config: &CacheConfig) -> Result<Self> {
        let redis_client = redis::RedisClient::new(&config.redis.url, config.redis.pool_size).await?;
        Ok(Self {
            redis: redis_client,
            config: config.clone(),
        })
    ...
    pub async fn cache_block(&self, chain_id: u64, number: u64, hash: &str, block: &Value) {
        ...
        self.redis.set(&num_key, &value, std::time::Duration::from_secs(60)).await;

        // Cache by hash (longer TTL, block hash is immutable unless reorg)
        let hash_key = keys::block_by_hash_key(chain_id, hash);
        self.redis.set(&hash_key, &value, std::time::Duration::from_secs(3600)).await;
    ...
    pub async fn cache_logs(&self, chain_id: u64, block_number: u64, logs: &Value) {
        ...
        self.redis.set(&key, &value, std::time::Duration::from_secs(60)).await;
