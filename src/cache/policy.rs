...
mod tests {
    ...
    fn test_resolve_ttl_exact_method_match() {
        let config = make_config(vec![
            make_policy(&["eth_getBlockByHash"], Finality::Finalized, TtlValue::Forever),
        ]);
    ...
    fn test_resolve_ttl_never_cache() {
        let config = make_config(vec![
            make_policy(&["eth_sendRawTransaction"], Finality::Any, TtlValue::Never),
        ]);
    ...
    fn test_resolve_ttl_duration() {
        let config = make_config(vec![
            make_policy(&["eth_getBalance"], Finality::Unfinalized, TtlValue::Duration(Duration::from_secs(2))),
        ]);
    ...
    fn test_resolve_ttl_wildcard_match() {
        let config = make_config(vec![
            make_policy(&["*"], Finality::Finalized, TtlValue::Duration(Duration::from_secs(3600))),
        ]);
    ...
    fn test_resolve_ttl_finality_mismatch_skips_policy() {
        let config = make_config(vec![
            make_policy(&["eth_getBalance"], Finality::Finalized, TtlValue::Forever),
        ]);
    ...
    fn test_resolve_ttl_any_finality_matches_both() {
        let config = make_config(vec![
            make_policy(&["eth_call"], Finality::Any, TtlValue::Never),
        ]);
    ...
    fn test_resolve_ttl_first_match_wins() {
        let config = make_config(vec![
            make_policy(&["eth_getBalance"], Finality::Unfinalized, TtlValue::Duration(Duration::from_secs(2))),
            make_policy(&["*"], Finality::Unfinalized, TtlValue::Duration(Duration::from_secs(10))),
        ]);
        let ttl = resolve_ttl(&config, "eth_getBalance", true);
    ...
    fn test_matches_method_exact() {
        assert!(matches_method(&["eth_getBalance".to_string()], "eth_getBalance"));
        assert!(!matches_method(&["eth_getBalance".to_string()], "eth_getCode"));
    }
    ...
    fn test_matches_finality_exact() {
        ...
        assert!(matches_finality(Finality::Unfinalized, Finality::Unfinalized));
        assert!(!matches_finality(Finality::Finalized, Finality::Unfinalized));
    }
