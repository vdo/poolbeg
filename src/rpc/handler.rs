...
pub async fn handle_rpc(
    ...
    let response = match request {
        RpcRequest::Single(req) => {
            let resp = process_single_request(chain_mgr, &state.cache, chain_name, &state.config.chains[chain_idx], req).await;
        ...
        if let Some(cached) = cache.get(chain_config.chain_id, &req).await {
            ...
            let mut resp: JsonRpcResponse = match serde_json::from_str(&cached) {
                ...
                Err(_) => {
                    ...
                    return forward_to_upstream(chain_mgr, cache, chain_name, chain_config, req, original_id).await;
                }
            };
            resp.id = original_id;
            return resp;
    ...
    match chain_mgr.forward_request(&req).await {
        Ok(mut resp) => {
            ...
            if !is_uncacheable(&method) && resp.error.is_none() {
                ...
                cache.set(chain_config.chain_id, &req, &resp, is_unfinalized, &chain_config.name).await;
            }
