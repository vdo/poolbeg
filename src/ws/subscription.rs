    ...
    pub fn from_value(v: &Value) -> Self {
        let address = v.get("address").and_then(|a| {
            if let Some(s) = a.as_str() {
                ...
                Some(arr.iter().filter_map(|x| x.as_str().map(|s| s.to_lowercase())).collect())
            } else {
                None
            }
        });
            ...
            for (i, topic_filter) in topics.iter().enumerate() {
                if let Some(ref filter_values) = topic_filter {
                    match log_topics.get(i) {
                        Some(log_topic) => {
                            if !filter_values.iter().any(|f| f.eq_ignore_ascii_case(log_topic)) {
                                return false;
                            }
                        }
                        None => return false,
...
impl SubscriptionManager {
    ...
    pub async fn subscribe(
        ...
        self.subscriptions.write().await.insert(subscription_id.clone(), sub);
    ...
    async fn handle_event(&self, event: BlockEvent) {
        match event {
            BlockEvent::NewBlock { chain_id, header, .. } => {
...
mod tests {
    ...
    fn test_log_filter_from_value_multiple_addresses() {
        ...
        let filter = LogFilter::from_value(&v);
        assert_eq!(filter.address, Some(vec!["0xabc".to_string(), "0xdef".to_string()]));
    }
    ...
    fn test_log_filter_matches_no_filter() {
        let filter = LogFilter { address: None, topics: None };
    ...
    fn test_log_filter_or_topics() {
        ...
        let filter = LogFilter {
            address: None,
            topics: Some(vec![
                Some(vec!["0xtransfer".to_string(), "0xapproval".to_string()]),
            ]),
        };
