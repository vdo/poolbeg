...
impl BlockTracker {
    ...
    pub async fn run(
        ...
        loop {
            ...
            // Fetch latest block
            match self.fetch_latest_block(&upstream, &cache, &mut last_block_number, &mut last_block_hash).await {
                Ok(()) => {}
                Err(e) => {
    ...
    async fn fetch_latest_block(
        ...
        if number <= *last_block_number && hash != *last_block_hash && *last_block_number > 0 {
            ...
            cache.invalidate_head_cache(self.chain_id, &self.chain_name).await;
        }
...
fn select_best_upstream(upstreams: &[Arc<UpstreamClient>]) -> Option<Arc<UpstreamClient>> {
    ...
    let mut sorted: Vec<_> = upstreams.iter().filter(|u| u.is_healthy()).cloned().collect();
