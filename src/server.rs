...
pub async fn setup(config: Config) -> anyhow::Result<Router> {
    ...
    for (idx, chain_config) in config.chains.iter().enumerate() {
        ...
        app = app.route(&route, post(rpc::handler::handle_rpc).with_state((state.clone(), idx)));
        // WebSocket endpoint on same route
        app = app.route(&route, get(ws::handler::handle_ws).with_state((state.clone(), idx)));
    }
...
pub async fn run(config: Config) -> anyhow::Result<()> {
    ...
    let metrics_addr = format!("{}:{}", config.server.metrics.address, config.server.metrics.port);
    let metrics_router = Router::new()
        .route(&config.server.metrics.path, get(app_metrics::metrics_handler));
